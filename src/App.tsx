...
function lsSet(key: string, val: string) { ary { window.localStorage.setItem(key, val); } catch {} }
...
export default function App() {
  ...
  // Supports:
  //  - /receipt?code=ID&d=ENCODED   (preferred)
  //  - /?receipt=ID&d=ENCODED       (legacy)
  //  - /receipt?code=ID             (admin preview: loads from localStorage if available)
  if (typeof window !== "undefined") {
    const url = new URL(window.location.href);
    const params = url.searchParams;

    const receiptId = params.get("code") || params.get("receipt");
    ...
    if (receiptId && isReceiptPath) {
      // 1) Prefer encoded payload (public/shareable)
      if (encoded) {
        try {
      ...
      // 2) Fallback: try localStorage (useful for admin preview on same device)
      try {
        const existing = loadBatches();
        const found = existing.find((b) => b.id === receiptId);
        if (found) return <PublicReceipt batch={found} />;
      } catch {}

      // 3) If we got here, the link is incomplete (no payload + not found locally)
      return (
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f6f6f6", padding: 24 }}>
          <div style={{ maxWidth: 680, width: "100%", background: "white", border: "1px solid #e6e6e6", borderRadius: 16, padding: 20, boxShadow: "0 10px 30px rgba(0,0,0,.06)" }}>
            <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 6 }}>Receipt link incomplete</div>
            <div style={{ color: "#444", lineHeight: 1.45 }}>
              This receipt link contains an ID (<b>{receiptId}</b>) but no receipt payload.
              <br />
              If you scanned a QR, regenerate it so it points to:
              <br />
              <code style={{ background: "#f3f3f3", padding: "2px 6px", borderRadius: 8 }}>/receipt?code=&lt;ID&gt;&amp;d=&lt;ENCODED&gt;</code>
              <br />
              If you’re testing locally, open the receipt from the app’s “Public link” button so it includes <code style={{ background: "#f3f3f3", padding: "2px 6px", borderRadius: 8 }}>d=</code>.
            </div>
            <div style={{ display: "flex", gap: 10, marginTop: 14 }}>
              <a href="/" style={{ textDecoration: "none" }}>
                <button style={{ padding: "10px 12px", borderRadius: 12, border: "1px solid #ddd", background: "#fff", fontWeight: 700, cursor: "pointer" }}>
                  Back to app
                </button>
              </a>
            </div>
...
function BatchDetail({batch,updateBatch,deleteBatch,archiveBatch,unarchiveBatch,closeBatch,addToast,speciesLibrary,companyLibrary}:any) {
  ...
  const publicUrl = `${RESOLVER_BASE}/receipt?code=${batch.id}&d=${encoded}`;
